# R script to answer problem set 1
# Group composition: Emanuele Artoni, Pedro Cassandra, and Arturs Janis Nikitins
...
# Adding past outcomes (regression_f3), the point estimate inverts (213 1982 dollar increase in '78 real earnings, ceteris paribus) and becomes statistically insignificant.
...
# the double selection procedure helps ensure that variables relevant for treatment
# assignment are also included. This makes the estimator more robust to imperfect model selection.
# Concretely, the selection of several age and age-education interaction terms in the treatment
# equation indicates that treatment assignment, in this sample, is statistically related to specific
# demographic characteristics. These imbalances between treatment and control units could be considered to be due to chance as the underlying assignment was randomized.
...
# This results in more treated units exceeding the 0.8 threshold (59 (31.9%) trimmed treated observations for RF vs 37 (20%) for logit), so the trimmed sample depends on the choice of estimator.
