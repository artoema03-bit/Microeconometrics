...
TABLE_3 <- data.frame(
  reg_78_full  = reg_extractor(reg_78_full),
  reg_78_logit = reg_extractor(reg_78_logit),
  reg_78_rf    = reg_extractor(reg_78_rf),
  reg_75_full  = reg_extractor(reg_75_full),
  reg_75_logit = reg_extractor(reg_75_logit),
  reg_75_rf    = reg_extractor(reg_75_rf)
)
...
## RF is still reproducible, given a seed an tuning parameters, but the process is less transparent.
